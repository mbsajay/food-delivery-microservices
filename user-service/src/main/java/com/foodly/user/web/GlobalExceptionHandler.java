...
/**
 * Translates exceptions into the shared {@link ApiResponse} envelope so every
 * Foodly service answers errors with the same shape. HTTP status is taken from
 * the {@link ErrorCode} carried by each {@link FoodlyException}.
 */
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handleFoodly(FoodlyException ex, HttpServletRequest request) {
        ApiError error = baseError(ex.getErrorCode(), ex.getMessage(), request).build();
        return respond(ex.getErrorCode(), error);
    }
    ...
    public ResponseEntity<ApiResponse<Object>> handleUnexpected(Exception ex, HttpServletRequest request) {
        ApiError error = baseError(ErrorCode.INTERNAL_ERROR, "Unexpected error", request).build();
        return respond(ErrorCode.INTERNAL_ERROR, error);
    }
